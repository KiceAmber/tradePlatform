package com.kice.controller.upload;
...
public class FileServlet extends javax.servlet.http.HttpServlet{
    ...
    public static DiskFileItemFactory getDiskFileItemFactory(File file) {//即使不设置也有默认值
    ...
    public static ServletFileUpload getServletFileUpload(DiskFileItemFactory factory) {//即使不设置也有默认值
    ...
    public static String uploadParseRequest(ServletFileUpload upload,HttpServletRequest request,String uploadPath) throws FileUploadException, IOException {
        ...
        //3.把前端请求解析，封装成一个FileItem对象，需要从ServletFileUpload中获取
        ...
        for (FileItem fileItem : fileItems) {
            if (fileItem.isFormField()){ //判断上传的文件是普通的表单还是带文件的表单
                ...
                if (uploadFileName.trim().equals("")||uploadFileName==null){//可能存在文件名不合法
                    continue;
                }

                //获得上传的文件名  /images/girl/paojie.png
                //获取文件上传名就是拿到整个文件的路径，在最后一个/+1也就是最后一个/之后的内容
                String fileName = uploadFileName.substring(uploadFileName.lastIndexOf("/") + 1);
                //获得文件的后缀名
                String fileExtName = uploadFileName.substring(uploadFileName.lastIndexOf(".") + 1);//最后一个.再+1
                    /*
                ...
                String uuidPath = UUID.randomUUID().toString();//处理安全问题，保证文件唯一
